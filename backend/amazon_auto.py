...
# Slack webhook for sending discount rankings (set in .env; do not commit the URL)
SLACK_WEBHOOK_URL = os.getenv('SLACK_WEBHOOK_URL', '').strip() or None

# Amazon product page URL base (Japanese site)
AMAZON_PRODUCT_URL_BASE = "https://www.amazon.co.jp/dp"
...
# Only save to spreadsheet when discount rate is above this (15% or higher; do not save 14% or lower)
MIN_DISCOUNT_PERCENT_FOR_SHEET = 15
...
# Clicking the div container for 15% discount (value="businessSavingFilter15")
DISCOUNT_15_PERCENT_RADIO = "xpath=/html/body/div[1]/div[1]/div/div/div[3]/section/div/div/div/div/div[1]/div[2]/div[2]/div[2]/div[2]/fieldset/div[3]"
...
def initialize_google_sheets():
    ...
    try:
        ...
        # Open the spreadsheet (must be shared with your Google account for edit access)
        spreadsheet = gc.open_by_key(SPREADSHEET_ID)
        worksheet = spreadsheet.sheet1  # Use first sheet
        print(f"[INFO] Spreadsheet opened: {spreadsheet.title} (ID: {SPREADSHEET_ID})")
        
        # Read existing data to find the last row number
        existing_data = worksheet.get_all_values()
        
        # Prepare header row (J = product image via =IMAGE, K = product URL; no column L)
        headers = [
            "No",
            "created_time",
            "ASIN",
...
def append_product_to_sheets(worksheet, product_rows, current_number, start_row=None):
    """
    Write one row per product to Google Sheets: the tier with the highest discount rate (column H).
    Only considers tiers where discount rate is >= MIN_DISCOUNT_PERCENT_FOR_SHEET (15% or higher).
    Creation time, product name, ASIN, product image, and product URL are kept as-is for that row.
    Uses explicit row range when start_row is given so data appears from row 2 onward reliably.
    ...
    Args:
        ...
        start_row: First row index to write (1-based). If given, uses update() for reliable display.

    Returns:
        If start_row is None: next product number or None if failed.
        If start_row is given: (next_product_number, next_start_row) or (None, start_row) if failed.
    """
    try:
        # Only save tiers with discount rate >= 15%
        filtered_rows = []
        for product in product_rows:
            rate = _parse_discount_rate(product.get('discount_rate', ''))
            if rate is not None and rate >= MIN_DISCOUNT_PERCENT_FOR_SHEET:
                filtered_rows.append(product)
        if not filtered_rows:
            if start_row is not None:
                return None, start_row  # No rows written; do not count as success
            return current_number

        # Keep only the row with the highest discount rate (column H) per product
        best = max(filtered_rows, key=lambda p: _parse_discount_rate(p.get('discount_rate', '')) or 0)
        # Identity fields (created_time, ASIN, name, image_url) are only set on the first tier (idx==0)
        first_tier = product_rows[0] if product_rows else best
        asin = (first_tier.get('asin', '') or best.get('asin', '') or '').strip()
        product_url = f"{AMAZON_PRODUCT_URL_BASE}/{asin}" if asin else ''
        image_url = (first_tier.get('image_url', '') or best.get('image_url', '') or '').strip()
        if image_url and str(image_url).startswith('http'):
            image_formula = '=IMAGE("' + str(image_url).replace('"', '""') + '")'
        else:
            image_formula = ''
        created_time = first_tier.get('created_time', '') or best.get('created_time', '')
        name = first_tier.get('name', '') or best.get('name', '')
        # J = product image (=IMAGE), K = product URL; no column L
        row = [
            current_number,
            created_time,
            asin,
            ...
            best.get('unit_price', ''),
            best.get('discount_rate', ''),
            best.get('discount_amount', ''),
            image_formula,   # J: display image from URL
            product_url      # K: exact product URL
        ]
        rows = [row]

        try:
            if start_row is not None:
                # Explicit range A2:K (11 columns; no L)
                end_row = start_row + len(rows) - 1
                range_a1 = f"A{start_row}:K{end_row}"
                worksheet.update(range_a1, rows, value_input_option='USER_ENTERED')
                next_row = end_row + 1
                return current_number + 1, next_row
            else:
                worksheet.append_rows(rows, value_input_option='USER_ENTERED')
                return current_number + 1
        except Exception as append_err:
            print(f"    [ERROR] Failed to write to Google Sheets: {append_err}")
            print(f"    [INFO] Ensure the spreadsheet is shared with your Google account (edit access): {SPREADSHEET_URL}")
            if start_row is not None:
        ...
        print(f"    [INFO] Ensure the spreadsheet is shared with your Google account: {SPREADSHEET_URL}")
        if start_row is not None:
            return None, start_row
        return None


def _parse_discount_rate(s):
    """Parse discount rate string (e.g. '30.5%') to float, or None if invalid."""
...
def _parse_discount_amount(s):
    """Parse discount amount string (e.g. '¥1,234') to float, or None if invalid."""
    if not s or not isinstance(s, str):
        return None
    s = s.strip().replace('¥', '').replace(',', '').replace('円', '').strip()
    try:
        return float(s)
    except (ValueError, TypeError):
        return None
...
def _format_quantity_display(qty):
    """Display quantity for Slack: e.g. '10+' -> '10個以上', '1' -> '1個'."""
    if not qty:
        return ""
    s = str(qty).strip()
    if s.endswith("+"):
        n = s[:-1].strip()
        return f"{n}個以上" if n else s
    if s == "1":
        return "1個"
    return f"{s}個" if s.isdigit() else s


def analyze_spreadsheet_rankings(worksheet):
    """
    Analyze spreadsheet data and return three top-5 rankings.

    Returns:
        dict with keys:
          - by_discount_rate_per_asin: list of (asin, rate, quantity)
          - by_discount_rate_with_qty: list of (asin, rate, quantity)
          - by_discount_amount: list of (asin, amount, quantity)
    """
    result = {
        'by_discount_rate_per_asin': [],
        'by_discount_rate_with_qty': [],
        'by_discount_amount': []
    }
    try:
        data = worksheet.get_all_values()
        # Fetch formulas so we can extract image URL from J (=IMAGE("url")) for Slack thumb_url
        try:
            data_formula = worksheet.get_all_values(value_render_option=ValueRenderOption.formula)
        except (TypeError, AttributeError):
            data_formula = data  # fallback if gspread version doesn't support value_render_option
    except Exception as e:
        print(f"[ERROR] Failed to read spreadsheet: {e}")
        return result

    if len(data) < 2:
        return result  # Headers only or empty

    # Resolve column indices (J = 商品画像 =IMAGE, K = 商品URL; no L)
    header_row = [str(h).strip() for h in data[0]]
    col_asin = header_row.index('ASIN') if 'ASIN' in header_row else 2
    col_qty = header_row.index('商品数') if '商品数' in header_row else 4
    col_rate = header_row.index('割引率（％）') if '割引率（％）' in header_row else 7
    col_amount = header_row.index('割引額（円）') if '割引額（円）' in header_row else 8
    col_product_url = header_row.index('商品URL') if '商品URL' in header_row else 10
    col_image = header_row.index('商品画像') if '商品画像' in header_row else 9

    def _image_url_from_formula(cell_value):
        """Extract URL from =IMAGE("url") or =IMAGE('url') formula. Handles "" inside double-quoted URL."""
        if not cell_value or not isinstance(cell_value, str):
            return ''
        # Double-quoted: allow "" (escaped quote) inside the URL
        m = re.search(r'=IMAGE\s*\(\s*"((?:[^"]|"")*)"\s*\)', cell_value, re.IGNORECASE)
        if m:
    ...
    # Build rows from sheet (one row per product); parse J formula for Slack thumb_url
    rows = []
    for i in range(1, len(data)):
        row = data[i]
        if len(row) <= max(col_asin, col_qty, col_rate, col_amount, col_product_url):
            continue
        asin = str(row[col_asin] or '').strip()
        if not asin:
            continue
        quantity = str(row[col_qty] or '').strip()
        rate_str = row[col_rate] if len(row) > col_rate else ''
        amount_str = row[col_amount] if len(row) > col_amount else ''
        rate = _parse_discount_rate(rate_str)
        amount = _parse_discount_amount(amount_str)
        product_url = str(row[col_product_url] or '').strip() if len(row) > col_product_url else ''
        # Extract image URL from column J (=IMAGE("url")) for Slack thumb_url
        formula_row = data_formula[i] if i < len(data_formula) else []
        image_url = ''
        if len(formula_row) > col_image:
            image_url = _image_url_from_formula(formula_row[col_image] or '')
        rows.append({
            'asin': asin,
            'quantity': quantity,
            'discount_rate': rate,
    ...
    # 1. 単品のみ: Top 5 by discount rate among rows where quantity is 1 (single product only)
    single_product_rows = [r for r in rows if (r.get('quantity') or '').strip() == '1' and r['discount_rate'] is not None]
    asin_best_rate = {}
    for r in single_product_rows:
        asin = r['asin']
        if asin not in asin_best_rate or (r['discount_rate'] or 0) > asin_best_rate[asin]['rate']:
            asin_best_rate[asin] = {'rate': r['discount_rate'], 'quantity': '1', 'image_url': r.get('image_url', '')}
    sorted_by_rate = sorted(asin_best_rate.items(), key=lambda x: -x[1]['rate'])[:5]
    result['by_discount_rate_per_asin'] = [(asin, data['rate'], data['quantity'], data.get('image_url', '')) for asin, data in sorted_by_rate]

    # 2. Top 5 by discount rate with quantity (each row is ASIN+Quantity)
    rows_with_rate = [r for r in rows if r['discount_rate'] is not None]
    rows_with_rate.sort(key=lambda r: -(r['discount_rate'] or 0))
    result['by_discount_rate_with_qty'] = [(r['asin'], r['discount_rate'], r['quantity'], r.get('image_url', '')) for r in rows_with_rate[:5]]

    # 3. Top 5 rows by column I (割引額) — examine each row, no dedup by ASIN
    rows_with_amount = [r for r in rows if r['discount_amount'] is not None and r['discount_amount'] > 0]
    rows_with_amount.sort(key=lambda r: -(r['discount_amount'] or 0))
    result['by_discount_amount'] = [(r['asin'], r['discount_amount'], r['quantity'], r.get('image_url', '')) for r in rows_with_amount[:5]]

    return result


# Slack attachment colors by rank (priority): gold, silver, bronze, gray, dark gray
_SLACK_RANK_COLORS = ("#FFD700", "#C0C0C0", "#CD7F32", "#9E9E9E", "#757575")


def _amazon_product_link(asin):
    """Return Slack mrkdwn link for Amazon product page (ASIN clickable)."""
    if not asin:
        return ""
    url = f"{AMAZON_PRODUCT_URL_BASE}/{asin}"
    return f"<{url}|{asin}>"


def _amazon_product_url(asin):
    """Return full product URL for display/copy."""
    if not asin:
        return ""
    return f"{AMAZON_PRODUCT_URL_BASE}/{asin}"


def _slack_add_section(attachments, title, entries, fmt_line):
    """Append one ranking section. Entry = (asin, rate/amount, quantity, image_url). Product URL and image URL in text (raw URLs so Slack auto-links); thumb_url for avatar-sized image."""
    attachments.append({"color": "#2c3e50", "title": title})
    for idx, entry in enumerate(entries[:5]):
        color = _SLACK_RANK_COLORS[idx] if idx < len(_SLACK_RANK_COLORS) else "#757575"
        text = fmt_line(idx + 1, entry)
        image_url = (entry[3] if len(entry) > 3 else "") or ""
        att = {"color": color, "text": text, "mrkdwn_in": ["text"]}
        if image_url and str(image_url).strip().startswith("http"):
            att["thumb_url"] = image_url.strip()  # Avatar-sized product image in Slack
            att["fallback"] = text[:200] + ("..." if len(text) > 200 else "")
        attachments.append(att)
    if not entries:
        attachments.append({"color": "#9E9E9E", "text": "データなし", "mrkdwn_in": ["text"]})


def _build_rank_blocks_line(rank, entry, fmt_line):
    """
    Build Block Kit blocks for one ranking row: section (text + image on right) + actions (button to open product page).
    entry = (asin, rate_or_amount, quantity, image_url).
    Returns list of blocks (section, optional actions).
    """
    asin = entry[0] if entry else ""
    image_url = (entry[3] if len(entry) > 3 else "") or ""
    product_url = _amazon_product_url(asin)
    text_line = fmt_line(rank, entry).replace("\n", " • ").strip()  # single line for section
    blocks = []
    # Section: left = rank/ASIN/rate/quantity; right = image (accessory)
    section = {
        "type": "section",
        "text": {"type": "mrkdwn", "text": text_line},
    }
    if image_url and str(image_url).strip().startswith("http"):
        section["accessory"] = {
            "type": "image",
            "image_url": image_url.strip(),
            "alt_text": f"商品画像 {asin or rank}",
        }
    blocks.append(section)
    # Button: 商品ページを開く → opens Amazon product page
    if product_url:
        blocks.append({
            "type": "actions",
            "elements": [
                {
                    "type": "button",
                    "text": {"type": "plain_text", "text": "商品ページを開く", "emoji": True},
                    "url": product_url,
                    "action_id": f"open_amazon_{rank}_{asin or 'n'}",
                }
            ],
        })
    return blocks
...
def _build_rankings_blocks(rankings):
    """
    Build full Block Kit blocks for ランキング5件 × 各行右に画像, with 商品ページを開く button per row.
    rankings: dict with by_discount_rate_with_qty, by_discount_amount (each list of (asin, rate/amount, qty, image_url)).
    """
    blocks = []

    def fmt_with_qty_rate(rank, e):
        qty_disp = _format_quantity_display((e[2] or "").strip()) if len(e) >= 3 and (e[2] or "").strip() else ""
        body = f"{rank}位: {e[0]} {_format_rate(e[1])}" + (f" 数量{qty_disp}" if qty_disp else "")
        return body + " • " + _amazon_product_link(e[0])

    def fmt_with_qty_amount(rank, e):
        qty_disp = _format_quantity_display((e[2] or "").strip()) if len(e) >= 3 and (e[2] or "").strip() else ""
        body = f"{rank}位: {e[0]} {_format_amount(e[1])}" + (f" 数量{qty_disp}" if qty_disp else "")
        return body + " • " + _amazon_product_link(e[0])

    # Header
    blocks.append({"type": "header", "text": {"type": "plain_text", "text": "割引ランキング（各5件）", "emoji": True}})
    blocks.append({"type": "divider"})

    # 数量別商品割引率ランキング
    blocks.append({"type": "header", "text": {"type": "plain_text", "text": "数量別商品割引率ランキング", "emoji": True}})
    entries_rate = rankings.get("by_discount_rate_with_qty", [])[:5]
    if not entries_rate:
        blocks.append({"type": "section", "text": {"type": "mrkdwn", "text": "_データなし_"}})
    else:
        for idx, entry in enumerate(entries_rate):
            blocks.extend(_build_rank_blocks_line(idx + 1, entry, fmt_with_qty_rate))

    blocks.append({"type": "divider"})

    # 割引額が大きい商品ランキング
    blocks.append({"type": "header", "text": {"type": "plain_text", "text": "割引額が大きい商品ランキング", "emoji": True}})
    entries_amt = rankings.get("by_discount_amount", [])[:5]
    if not entries_amt:
        blocks.append({"type": "section", "text": {"type": "mrkdwn", "text": "_データなし_"}})
    else:
        for idx, entry in enumerate(entries_amt):
            blocks.extend(_build_rank_blocks_line(idx + 1, entry, fmt_with_qty_amount))

    blocks.append({"type": "divider"})
    blocks.append({
        "type": "section",
        "text": {"type": "mrkdwn", "text": f"<{SPREADSHEET_URL}|スプレッドシートを開く>"},
    })

    return blocks


def send_rankings_to_slack(rankings):
    """Send top 5 products per section using Block Kit: ランキング5件 × 各行右に画像, with 商品ページを開く button per row."""
    if not SLACK_WEBHOOK_URL:
        print("[WARNING] Slack webhook URL not configured, skipping Slack notification")
        return False

    blocks = _build_rankings_blocks(rankings)
    payload = {
        "text": "割引ランキング（各5件）",  # fallback for notifications / legacy clients
        "blocks": blocks,
    }
    ...
    try:
        ...
        if resp.status_code != 200:
            print(f"[ERROR] Slack webhook failed: {resp.status_code} - {resp.text[:200]}")
            return False
        print("[SUCCESS] Rankings sent to Slack (Block Kit: 5 rows × image right, 商品ページを開く button)")
        return True
    except Exception as e:
...
def clear_sheets_data(worksheet, keep_headers=True):
    """Clear all data rows from the worksheet. Optionally keep the header row."""
    try:
        if keep_headers:
            worksheet.batch_clear(["A2:K10000"])  # Columns A-K (商品画像, 商品URL); no L, keep row 1
        else:
            worksheet.clear()
        print("[SUCCESS] Spreadsheet data cleared")
        return True
    except Exception as e:
        print(f"[ERROR] Failed to clear spreadsheet: {e}")
        return False
...
def scrape_product_from_listing(container):
    ...
    try:
        ...
        # ===== Extract Product Image URL (main image) =====
        image_url = ''
        try:
            img_selectors = [
            ...
            for selector in img_selectors:
                img_elem = container.locator(selector).first
                if img_elem.count() > 0:
                    image_url = img_elem.get_attribute('src') or ''
                    if image_url and 'media-amazon.com' in image_url:
                        break
        except Exception as e:
            print(f"    [DEBUG] Product image extraction error: {e}")
        ...
        for idx, tier in enumerate(quantity_tiers):
            ...
            product_data = {
                ...
                'image_url': image_url  # Main product image (same for all tiers)
            }
...
def scrape_all_products(page, worksheet, current_number):
    ...
    try:
        ...
        next_row = 2  # Write data from row 2 onward (row 1 = headers) for reliable display
        scroll_count = 0
            ...
            for container in product_containers:
                try:
                    ...
                    if product_rows:
                        ...
                        # Send to Google Sheets IMMEDIATELY (explicit row range so data appears from row 2)
                        new_number, next_row = append_product_to_sheets(worksheet, product_rows, current_number, start_row=next_row)
                        
                        if new_number is not None:
                            # Success! (one row per product: highest discount rate only)
                            new_products_found += 1
                            total_rows_sent += 1
                            current_number = new_number
                            best_tier = max(product_rows, key=lambda p: _parse_discount_rate(p.get('discount_rate', '')) or 0)
                            print(f"  ✓ [{current_number - 1}] {asin} - {product_name[:50]}...")
                            print(f"     数量 {best_tier.get('quantity', '?')} (最高割引) → 1 row SENT")
                        else:
...
def apply_filters_and_sort(page):
    ...
    try:
        ...
        # No need to scroll for 15% option
        print("\n[INFO] Discount modal opened, 15% option should be visible...")
        try:
            # Wait a moment for modal to fully render
            time.sleep(1)
        ...
        # Select 15% discount radio button
        print("\n[4/5] Selecting 15% discount filter...")
        discount_15_radio = page.locator(DISCOUNT_15_PERCENT_RADIO)
        if discount_15_radio.count() > 0:
            try:
                # Scroll element into view before clicking
                discount_15_radio.scroll_into_view_if_needed(timeout=3000)
                time.sleep(0.3)
                # Click the div container for 15% discount
                discount_15_radio.click(force=True, timeout=3000)
                time.sleep(0.5)
                print("[SUCCESS] 15% discount filter selected")
            except Exception as e:
                print(f"[ERROR] Failed to select 15% discount: {e}")
        else:
            print("[WARNING] 15% discount radio button not found")
...
def run_automation():
    ...
    with sync_playwright() as p:
        try:
            ...
            print("\n" + "="*60)
            print("STEP: RESETTING GOOGLE SHEETS DATA")
            print("="*60)
            cleared = clear_sheets_data(worksheet, keep_headers=True)
            if cleared:
                current_number = 1
                print("[INFO] Spreadsheet reset completed. Starting from product number 1.")
            else:
                print("[WARNING] Failed to reset spreadsheet before scraping. Existing rows will be appended.")
            ...
            # Close browser before Slack/sheets operations
            print("\n[INFO] Closing browser...")
            browser.close()
            print("[SUCCESS] Browser closed")

            # Analyze spreadsheet, send rankings to Slack
            print("\n" + "="*60)
            print("STEP: ANALYZE & SEND TO SLACK")
            print("="*60)
            time.sleep(2)  # Allow Google Sheets to propagate before reading
            rankings = analyze_spreadsheet_rankings(worksheet)
            n_qty = len(rankings["by_discount_rate_with_qty"])
            n_amt = len(rankings["by_discount_amount"])
            print(f"[INFO] Rankings: 数量別={n_qty}, 割引額={n_amt}")
            sent = send_rankings_to_slack(rankings)
            if not sent:
                print("[WARNING] Slack notification was not sent (check SLACK_WEBHOOK_URL and logs above)")
